...
### 2026-09-26 首页 UI 逐像素校准：数值化对比闭环（v24）

- **本地自截图对比闭环**：`tools/smoke/dbg2-rows.mjs` 本地起服+播种 8 房间+CDP 截图，`tools/optimize` 下一组数值对比脚本（`cmp-crops` 行带/按钮、`arb-cols` 列质心、`arb-lock` 锁贴片、`nav-diff` 导航区 MAE、`glyph-count` 字形簇）把渲染截图与效果图逐项量化 —— 最终行带 Δ1-2px、列质心 Δ≤5px、导航区 MAE=5.52（调色板量化噪声级）、锁 8/8 行同位
- **效果图直裁图纸 `lobby-elems.png` 扩到 15 元素**：新增玩家信息牌整牌 `playerBar2`（烧死名字/金币已两侧净列插值擦成动态位）、左导航整列 `nav`（金选中页签+深色连排）、底部「创建房间」绿钮/「快速加入」金钮整钮（自带文字）、右下吉祥物、**行首金锁贴片 `lock`**（效果图每行房间号左侧一枚 —— 此前「号下面有锁」实为摆放错位，不是多余元素）、加入胶囊（内置「加入」二字已擦除，改代码 Label 动态画，满员换「已满」不再双重叠字）
- **列几何全面校准**：5 个数据列（房间号/名称/底注/人数/状态）全部**居中对齐**在表头正下方（表头↔数据交叉验证 Δ<3px，推翻此前左对齐的记错值）
- **行文字改纯白**：效果图实测房间号/名称等为 (255,255,255) 纯白 —— 此前仿金色 (204,145,57) 正是「对比度不高」的根因；表头米金色 (238,219,182)
- **底注列改单数字**：效果图口径是 `10/25/…` 单个数字（字形簇测量：行1=窄1+宽0 两位），此前渲染成 `10/20` 区间 → 改显小额注
- **玩家信息牌**：整牌直裁 + 昵称白字动态字号（超长缩号）+ 金币金字，修掉双头像与文字错位
- **大按钮触摸反馈**：按下白色高亮闪现、松手 0.28s 淡出（UIOpacity 补间）
- **z 序修正**：不透明面板素材先画，玩家牌/导航/圆钮/吉祥物/底部钮等压边元素一律后画（面板会盖住背景烧死的同款元素，反之元素要压回面板边缘）
- **构建排障**：VSCode 终端继承 `ELECTRON_RUN_AS_NODE` 会让 CocosCreator.exe 变纯 Node 拒绝 `--project` → `env -u ELECTRON_RUN_AS_NODE` 解除；本次构建 settings 产出不带哈希名 → `patch-build.mjs` 正则放宽兼容两种命名
